...
-- iv. Produce a list of text books (include Course #, Book-ISBN, Book-title) in the alphabetical order for courses offered by the ‘CS’ department that use more than two books. 
    ...
    select b.book_isbn from book_adoption b where b.course in (select c.course from course c where c.dept="CS") and (select count(*) from book_adoption be where be.course=b.course)>=2 ; 
    insert into course values(66,"OS","CS");
    insert into text values(7, "OPERATING SYSTEM", "MC GRAW","ABRAHAM SILBERSCHATZ");
	insert into book_adoption values(66, 6 ,7);
    select * from book_adoption;
    -- v. List any department that has all its adopted books published by a specific publisher.
	
    select distinct c.dept from course c where not exists (select t.book_isbn from text t where t.publisher="SPRINGER") not in (select b.book_isbN from book_adoption b where b.course in 
    (select ce.course from course ce where ce.dept=c.dept));
